...
public class FaceLoginCommandHandler : IRequestHandler<FaceLoginCommand, Result<LoginResponse>>
{
    ...
    public FaceLoginCommandHandler(
    ...
    public async Task<Result<LoginResponse>> Handle(FaceLoginCommand request, CancellationToken cancellationToken)
    {
        if (request.FaceDescriptor == null || request.FaceDescriptor.Length == 0)
            return Result<LoginResponse>.Failure("Face descriptor is required.");
        ...
        foreach (var user in usersWithFaces)
        {
            try
            {
                ...
                if (enrolledDescriptor == null || enrolledDescriptor.Length != request.FaceDescriptor.Length)
                    continue;

                var distance = ComputeEuclideanDistance(request.FaceDescriptor, enrolledDescriptor);
